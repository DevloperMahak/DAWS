...
/**
 * Linear/Vercel style sidebar (collapsible on small screens)
 * - modern glassy look
 * - subtle hover glow with lc-orange accent in dark mode
 * - uses a local uploaded image as the logo (see logoUrl below)
 */

const logoUrl =
  "/mnt/data/A_promotional_digital_graphic_design_showcases_DAW.png"; // uploaded file path
    ...
    >
      <div className="flex items-center gap-3 px-2">
        <div className="w-10 h-10 rounded-lg overflow-hidden bg-white/10 flex items-center justify-center">
          {/* Logo: local file path used here */}
          <img
            src={logoUrl}
            alt="DAWS logo"
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `group flex items-center gap-3 px-3 py-2 rounded-lg mb-1 transition-all duration-150
               ${
                 isActive
                   ? "bg-gradient-to-r from-[#f89f23]/20 to-transparent border-l-4 border-[#f89f23] text-[#f89f23]"
                   : "text-gray-700 dark:text-[#cbd5e1] hover:bg-white/5 dark:hover:bg-white/5"
               }`
            }
          >
            <span className="w-6 h-6 text-gray-600 dark:text-[#8b949e] group-hover:text-[#f89f23]">
              {it.icon}
            </span>
